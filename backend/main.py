...
# --- THE FIX: NATIVE CREWAI STRING ---
# We do not use a custom LLM class. We pass this string directly to the agents.
# CrewAI will automatically detect os.environ.get("GEMINI_API_KEY") in Render.
...
async def health_check():
    return {
        ...
        "version": "10.1.0 (Native LLM Routing)"
    }
...
def get_hybrid_metrics(ticker: str) -> dict:
    ...
    # 1. YAHOO FINANCE NATIVE EXTRACTION (No custom session to avoid curl_cffi error)
    try:
    ...
    # 2. SCREENER.IN INDIAN FALLBACK EXTRACTION (Uses masked session)
...
def comprehensive_yfinance_data(ticker: str) -> str:
    ...
    try:
        stock = yf.Ticker(ticker) # Native YF call
...
async def analyze_stock(request: AnalyzeRequest):
    try:
        ...
        # ==========================================
        # AGENTS MASSIVELY EXPANDED FOR MAXIMUM PRECISION
        # FIX: llm string mapped directly to CrewAI LiteLLM backend
        # ==========================================
        ...
        research_agent = Agent(
            ...
            llm=GEMINI_MODEL_STRING, max_iter=3, verbose=True
        )
