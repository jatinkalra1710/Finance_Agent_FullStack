...
export default function App() {
  ...
  // --- FIXED METRICS FETCHING with Timeout & Better Error Handling ---
  const fetchLiveRatios = async (targetTicker) => {
    ...
    console.log('Fetching metrics for:', targetTicker);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data && data.metrics && Object.keys(data.metrics).length > 0) {
          ...
          setMetrics({ 
            "Status": "No Data",
            "Message": "Try different ticker"
          });
        }
      } else {
        console.error('Metrics fetch error:', response.status);
        setMetrics({ 
          "Status": "API Error",
  ...
  // Auto-fetch metrics 800ms after user stops typing
  useEffect(() => {
  ...
  return (
    <div className={`flex h-screen font-sans overflow-hidden relative selection:bg-blue-500/30 transition-colors duration-500 ${baseBg}`}>
      ...
      {/* --- ALL MODALS (keeping your original modals exactly as they are) --- */}
      ...
      {/* Continue with other modals... (keeping all your original modals) */}
      {/* I'll keep the rest of your modals exactly as they are */}
      ...
      <div className="flex-1 flex flex-col h-full relative z-10 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-8">
          ...
          {/* TAB: DASHBOARD (TERMINAL) - This is where the METRICS FIX is */}
          {activeTab === 'dashboard' && (
            <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in-up">
                ...
                <div className={`lg:col-span-4 ${cardBg} backdrop-blur-xl p-6 rounded-3xl shadow-sm border flex flex-col justify-between relative overflow-hidden group transition-colors duration-500`}>
                  <div className="relative z-10">
                    <h2 className={`font-bold text-lg mb-6 flex items-center gap-2 ${textHeading}`}>
                      <Target className="w-5 h-5 text-blue-500 animate-pulse" /> Target Asset Parameters
                    </h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[700px]">
                ...
                {/* --- ENHANCED RATIOS GRID with ALL METRICS --- */}
                <div className={`lg:col-span-4 h-full ${cardBg} backdrop-blur-xl rounded-3xl shadow-sm border overflow-hidden flex flex-col transition-colors duration-500`}>
                  <div className={`p-5 border-b ${theme==='dark'?'border-slate-800/80 bg-slate-900/80':'border-slate-200 bg-slate-50/80'} flex items-center justify-between`}>
                    ...
                    {/* Status Badge */}
                    {fetchingMetrics && (
                      <span className="text-[10px] bg-amber-500/10 text-amber-500 px-2 py-1 rounded-full font-bold uppercase border border-amber-500/20">
                        Fetching...
                      </span>
                    )}
                    {!fetchingMetrics && metrics && Object.keys(metrics).length > 0 && !metrics.Status && (
                      <span className="text-[10px] bg-emerald-500/10 text-emerald-500 px-2 py-1 rounded-full font-bold uppercase border border-emerald-500/20">
                        ✓ Live
                      </span>
                    )}
                    {!fetchingMetrics && !metrics && ticker && (
                      <span className="text-[10px] bg-blue-500/10 text-blue-500 px-2 py-1 rounded-full font-bold uppercase border border-blue-500/20">
                        Ready
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 p-5 overflow-y-auto custom-scrollbar">
                    ...
                    {!loading && !fetchingMetrics && ticker && metrics && metrics.Status && (
                       <div className="h-full flex flex-col items-center justify-center text-center">
                         ...
                         <p className={`text-sm font-bold ${textHeading} mb-2`}>{metrics.Status}</p>
                         <p className={`text-xs ${textMuted} mt-1 mb-4`}>{metrics.Action || metrics.Message || 'Market data unavailable'}</p>
                         <button
                           onClick={() => fetchLiveRatios(ticker)}
                           className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-xs transition-all"
                         >
                    ...
                    {/* SUCCESS STATE - Display ALL Metrics */}
                    {metrics && !metrics.Status && Object.keys(metrics).length > 0 && (
                      <div className="animate-fade-in-up">
                        
                        {/* Summary Card - Highlight Key Metrics */}
                        {(metrics["Current Price"] || metrics["Market Cap"]) && (
                          <div className={`${theme==='dark'?'bg-blue-900/20 border-blue-500/30':'bg-blue-50 border-blue-200'} border-2 rounded-xl p-4 mb-4`}>
                            <div className="flex items-center justify-between mb-2">
                              <span className={`text-[10px] font-bold ${textMuted} uppercase tracking-wider`}>Current Valuation</span>
                              <TrendingUp className="w-4 h-4 text-blue-500" />
                            </div>
                            {metrics["Current Price"] && (
                              <div className={`text-2xl font-black ${textHeading} mb-1`}>
                                {metrics["Current Price"]}
                              </div>
                            )}
                            {metrics["Market Cap"] && (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {Object.entries(metrics)
                            ...
                            .map(([key, val]) => {
                              ...
                              return (
                                <div key={key} className={`flex flex-col justify-center ${theme==='dark'?'bg-slate-800/40 border-slate-700/50':'bg-slate-50 border-slate-200'} p-3.5 rounded-xl border hover:border-blue-500/50 transition-colors group`}>
                                  <span className={`${textMuted} text-[10px] font-bold uppercase tracking-wider mb-1 group-hover:text-blue-500 transition-colors`}>{key}</span>
                                  <span className={`${valueColor} font-extrabold text-sm truncate`}>{val}</span>
                                </div>
                              );
                            })}
                        </div>
                        ...
                        {/* Data Source Footer */}
                        <div className={`mt-4 pt-3 border-t ${theme==='dark'?'border-slate-800':'border-slate-200'} flex items-center justify-between text-[10px] ${textMuted}`}>
                          ...
                          <button
                            onClick={() => fetchLiveRatios(ticker)}
                            className="font-bold hover:text-blue-500 transition-colors flex items-center gap-1"
                          >
