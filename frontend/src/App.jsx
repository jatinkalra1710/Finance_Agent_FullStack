  ...
  return (
    <div className={`flex h-screen font-sans overflow-hidden relative selection:bg-blue-500/30 transition-colors duration-500 ${baseBg}`}>
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full mix-blend-screen filter blur-[100px] animate-blob ${theme==='dark' ? 'bg-blue-900/20' : 'bg-blue-200/50'}`}></div>
        <div className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full mix-blend-screen filter blur-[100px] animate-blob animation-delay-2000 ${theme==='dark' ? 'bg-emerald-900/20' : 'bg-emerald-200/50'}`}></div>
      </div>
      ...
      <aside className={`w-72 ${cardBg} backdrop-blur-2xl hidden md:flex flex-col relative z-20 shadow-2xl transition-colors duration-500`}>
        ...
        <div className={`p-4 border-t ${theme==='dark'?'border-slate-800 bg-slate-900/30':'border-slate-200 bg-slate-50'}`}>
      ...
      <div className="flex-1 flex flex-col h-full relative z-10 overflow-hidden">
        ...
        <header className={`${cardBg} backdrop-blur-xl border-b sticky top-0 z-30 px-6 py-4 flex items-center justify-between shadow-sm transition-colors duration-500`}>
          ...
          <div className={`hidden md:flex items-center gap-2 ${theme==='dark'?'bg-slate-800/50 border-slate-700':'bg-slate-100 border-slate-200'} px-4 py-1.5 rounded-full`}>
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in-up">
                ...
                <div className={`lg:col-span-8 ${cardBg} backdrop-blur-xl p-6 rounded-3xl shadow-sm border transition-colors duration-500`}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
                    {POPULAR_STOCKS.map((stock) => (
                      <button key={stock.ticker} onClick={() => handleSelectPopular(stock)} className={`text-left p-4 rounded-2xl border transition-all duration-300 ${ticker === stock.ticker ? `border-blue-500 ${theme==='dark'?'bg-blue-900/40':'bg-blue-50'} shadow-sm` : `${theme==='dark'?'border-slate-800 bg-slate-950/50 hover:bg-slate-800 hover:border-slate-600':'border-slate-200 bg-slate-50 hover:bg-white hover:border-slate-300'}`}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[700px]">
                ...
                {/* Left: TV Chart (Theme Syncs automatically) */}
                <div className="lg:col-span-8 h-full">
                  <div className={`${cardBg} backdrop-blur-xl p-2 rounded-3xl shadow-sm border h-full overflow-hidden transition-colors duration-500`}>
                    <AdvancedRealTimeChart theme={theme} symbol={getTradingViewSymbol(ticker)} autosize allow_symbol_change={false} hide_side_toolbar={false} backgroundColor="transparent" />
                  </div>
                </div>
                ...
                <div className={`lg:col-span-4 h-full ${cardBg} backdrop-blur-xl rounded-3xl shadow-sm border overflow-hidden flex flex-col transition-colors duration-500`}>
                  <div className={`p-5 border-b ${theme==='dark'?'border-slate-800 bg-slate-900/80':'border-slate-200 bg-slate-50/80'} flex items-center justify-between`}>
                  ...
                  <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
                    ...
                    {loading && (
                      <div className="h-full flex flex-col gap-4">
                        {[...Array(8)].map((_, i) => (
                          <div key={i} className={`flex justify-between items-center ${theme==='dark'?'bg-slate-800/30 border-slate-800':'bg-slate-100/50 border-slate-200'} p-4 rounded-xl border`}>
                            <div className={`h-3 w-20 ${theme==='dark'?'bg-slate-700':'bg-slate-200'} rounded animate-pulse`}></div>
                            <div className={`h-4 w-16 ${theme==='dark'?'bg-slate-600':'bg-slate-300'} rounded animate-pulse delay-75`}></div>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {metrics && Object.keys(metrics).length > 0 && !loading && (
                      <div className="flex flex-col gap-3 animate-fade-in-up">
                        {Object.entries(metrics).map(([key, val]) => (
                          <div key={key} className={`flex justify-between items-center ${theme==='dark'?'bg-slate-800/50 border-slate-700/50 hover:bg-slate-800':'bg-slate-50 border-slate-200 hover:bg-slate-100'} p-4 rounded-xl border transition-colors`}>
                            <span className={`${textMuted} text-xs font-bold uppercase tracking-wider`}>{key}</span>
                ...
                <div className="w-full min-h-[600px] animate-fade-in-up">
                  ...
                  {loading && (
                    <div className={`${cardBg} backdrop-blur-xl rounded-3xl shadow-sm border flex flex-col md:flex-row overflow-hidden min-h-[600px] transition-colors`}>
                      <div className={`w-full md:w-1/2 p-10 border-b md:border-b-0 md:border-r ${theme==='dark'?'border-slate-800':'border-slate-200'} relative flex flex-col justify-center`}>
                        <div className="relative z-10 mb-10">
                          ...
                          <p className={`${textMuted} font-medium`}>Orchestrating 7 specialized institutional models. Estimated completion: 60-90s.</p>
                        </div>
                        <div className="space-y-5 relative z-10">
                          {[
                            ...
                            "Technical Agent mapping support/resistance...",
                            "Sentiment Agent parsing institutional flows...",
                            "Chief Risk Officer identifying vulnerabilities...",
                      ...
                      {/* AdSense Revenue Container */}
                      <div className={`w-full md:w-1/2 ${theme==='dark'?'bg-slate-950':'bg-slate-50'} p-10 flex flex-col items-center justify-center relative`}>
                        <span className={`absolute top-6 left-8 text-[10px] ${textMuted} uppercase tracking-widest font-extrabold flex items-center gap-2`}>
                          <Zap className="w-3 h-3"/> Sponsored Payload
                        </span>
                  ...
                  {report && !loading && (
                    <div className={`${cardBg} backdrop-blur-xl p-10 lg:p-14 rounded-3xl shadow-sm border flex flex-col relative transition-colors`}>
                      ...
                      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-8 border-b ${theme==='dark'?'border-slate-800':'border-slate-200'} gap-4`}>
                        <div>
                      ...
                      <div className={`prose ${theme==='dark'?'prose-invert':''} prose-lg max-w-none 
                        prose-headings:${textHeading} prose-headings:font-extrabold tracking-tight
                        prose-h1:text-3xl prose-h1:border-b ${theme==='dark'?'prose-h1:border-slate-800':'prose-h1:border-slate-200'} prose-h1:pb-4 prose-h1:mb-8
                        prose-h2:text-2xl prose-h2:mt-10 prose-h2:mb-6 prose-h2:text-blue-500
                        prose-h3:text-xl prose-h3:text-indigo-500
                        prose-p:${textHeading} prose-p:opacity-90 prose-p:leading-loose prose-p:font-medium
                        prose-a:text-blue-500 hover:prose-a:text-blue-400
                        prose-strong:${textHeading} prose-strong:font-black
                        prose-ul:list-none prose-ul:pl-0 prose-li:relative prose-li:pl-6 prose-li:my-3 prose-li:${textHeading} prose-li:opacity-90
                        ...
                        prose-blockquote:border-l-4 prose-blockquote:border-blue-500 ${theme==='dark'?'prose-blockquote:bg-slate-800/50':'prose-blockquote:bg-blue-50'} prose-blockquote:py-4 prose-blockquote:px-6 prose-blockquote:rounded-r-2xl prose-blockquote:not-italic prose-blockquote:${textHeading} prose-blockquote:font-medium prose-blockquote:shadow-inner text-base`}>
